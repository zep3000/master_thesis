"""Evaluate the four first-100 pipelines against assignment 79201188.

This module is deliberately separate from run.py.  The inference runner cannot
import this file and never sees the human export.  Evaluation uses the matching
conventions established by the earlier Venice comparison:
...
* one-to-one greedy matching, with people/groups matched only inside an already
...
LEDGER = HERE / "output" / "shared_request_ledger.jsonl"
...
def load_gold() -> dict[str, dict[str, Any]]:
    export = json.loads(GOLD.read_text(encoding="utf-8"))
    return {row["image_id"]: row["payload"] for row in export["annotations"] if row.get("assignment_code") == ASSIGNMENT_CODE and isinstance(row.get("payload"), dict)}


def load_predictions(run_name: str, pipeline: str) -> dict[str, dict[str, Any]]:
    path = HERE / "output" / run_name / pipeline / "completed.jsonl"
    return {row["image_id"]: row["annotation"] for row in read_jsonl(path) if row.get("ok") and isinstance(row.get("annotation"), dict)}


def request_costs(run_name: str, pipeline: str) -> dict[str, Any]:
    rows = [row for row in read_jsonl(LEDGER) if row.get("run_name") == run_name and row.get("pipeline") == pipeline]
    stage_counts = Counter(str(row.get("stage")) for row in rows)
    usage_totals: Counter[str] = Counter(); cost = 0.0
    for row in rows:
        usage = row.get("usage") or {}
        for field in ["prompt_tokens", "completion_tokens", "total_tokens"]:
            try: usage_totals[field] += int(usage.get(field) or 0)
            except (TypeError, ValueError): pass
        try: cost += float(usage.get("cost") or 0)
        except (TypeError, ValueError): pass
    return {"requests": len(rows), "successful_requests": sum(bool(row.get("ok")) for row in rows), "by_stage": dict(stage_counts), "usage": dict(usage_totals), "reported_cost_usd": cost or None}
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = ["# First-100 pipeline matrix evaluation", "", f"Generated: {report['generated_at']}", "", "Strict spatial matching uses IoU >= .20 for ads/people and >= .18 for groups. Lenient matching uses IoU >= .10/.10/.08 or 95% containment with IoU >= .02. Entities are matched one-to-one and people/groups only within a matched ad.", ""]
    for scope in ["all_100", "pilot_10", "holdout_90"]:
        lines.extend([f"## {scope}", "", "| Pipeline | Calls | Mode | Ad F1 | Person F1 | Group F1 | No-group specificity | False groups |", "|---|---:|---|---:|---:|---:|---:|---:|"])
        for pipeline in ["p1", "p2", "p3", "p4"]:
            item = report["pipelines"][pipeline]["scopes"][scope]
            for mode in ["strict", "lenient"]:
                detail = item["match_modes"][mode]; det = detail["detection"]; ng = detail["human_no_group"]
                lines.append(f"| {pipeline} | {report['pipelines'][pipeline]['cost']['requests']} | {mode} | {pct(det['ads']['f1'])} | {pct(det['people']['f1'])} | {pct(det['groups']['f1'])} | {pct(ng['page_group_free_specificity'])} | {ng['false_predicted_groups']} |")
        lines.append("")
    lines.extend(["## Interpretation notes", "", "- Exact-label accuracy is conditional on a spatially matched entity; end-to-end label recall also penalizes missed entities.", "- `naturalistic_illustration` and `stylized_illustration` in the older human export are mapped to the v1.17 `illustration` label for every pipeline.", "- Human `not_applicable` group smile intensity is treated as null because the v1.17 pipeline uses conditional nulls.", "- The no-group section is a standalone route diagnostic: it reports whether pipelines refrain from inventing groups and how well they retain individual faces on those pages.", ""])
    return "\n".join(lines)
...
def main() -> None:
    ...
    ids = [row["image_id"] for row in manifest[:100]]; scopes = {"all_100": ids, "pilot_10": ids[:10], "holdout_90": ids[10:]}
    gold = load_gold()
    report = {
        "schema_version": "qwen_first100_pipeline_matrix_evaluation_v1", "generated_at": iso_now(), "run_name": args.run_name,
        "assignment_code": ASSIGNMENT_CODE,
        "matching": {"strict_iou": STRICT_THRESHOLDS, "lenient_iou": LENIENT_THRESHOLDS, "containment": CONTAINMENT_THRESHOLD, "minimum_contained_iou": MIN_CONTAINED_IOU, "hierarchical_within_matched_ad": True},
        "pipelines": {},
    }
    for pipeline in ["p1", "p2", "p3", "p4"]:
        predicted = load_predictions(args.run_name, pipeline)
        report["pipelines"][pipeline] = {"cost": request_costs(args.run_name, pipeline), "predicted_pages": len(predicted), "scopes": {name: evaluate_scope(gold, predicted, scope_ids) for name, scope_ids in scopes.items()}}
    output_dir = HERE / "evaluation" / args.run_name; output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "evaluation.json"; md_path = output_dir / "evaluation.md"
