#!/usr/bin/env python3
"""Frozen one-request-per-page Venice annotation run for the first 30 pages."""
...
DEFAULT_OUTPUT = SCRIPT_DIR / "output" / "venice_first30_single_shot.jsonl"
DEFAULT_AGGREGATE = SCRIPT_DIR / "output" / "venice_first30_single_shot.aggregate.json"
DEFAULT_RAW_DIR = SCRIPT_DIR / "output" / "raw_responses"
DEFAULT_PROMPT_DIR = SCRIPT_DIR / "output" / "prompt_snapshots"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--app-name", default=os.getenv("OPENROUTER_APP_NAME", "economist-ad-face-venice-first30-single-shot"))
...
def main() -> int:
    ...
    with output_path.open(mode, encoding="utf-8") as handle:
        for run_index, item in enumerate(pending, start=1):
            ...
            record = annotate_one(client, item, image_path, args, raw_dir, prompt_dir)
...
def annotate_one(
    ...
    prompt_file = prompt_dir / f"{safe_stem(filename)}.prompt.txt"
    ...
    record: dict[str, Any] = {
        "schema_version": "qwen_venice_first30_single_shot_record_v1",
        "image_id": image_id,
        "filename": filename,
...
def write_aggregate(
    ...
    payload = {
        "schema_version": "qwen_venice_first30_single_shot_aggregate_v1",
        ...
        "data_isolation": {
            ...
            "request_inputs": ["sanitized_manifest", "original_page_image", "frozen_schema_and_prompt"],
        },
