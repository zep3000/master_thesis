...
SCHEMA_VERSION = "qwen_venice_app_v2_four_pass_v1"
PLAYBOOK_VERSION = "annotation_playbook_v1.16"
PROMPT_VERSION = "venice_app_v2_four_pass_prompt_v1"
GLOBAL_SCHEMA_VERSION = "qwen_venice_app_v2_four_pass_global_v1"
GLOBAL_PROMPT_VERSION = "venice_app_v2_four_pass_global_prompt_v1"
REPAIR_SCHEMA_VERSION = "qwen_venice_app_v2_four_pass_bbox_repair_v1"
REPAIR_PROMPT_VERSION = "venice_app_v2_four_pass_bbox_repair_prompt_v1"
CROP_SCHEMA_VERSION = "qwen_venice_app_v2_four_pass_crop_features_v1"
CROP_PROMPT_VERSION = "venice_app_v2_four_pass_crop_features_prompt_v1"
CORRECTION_SCHEMA_VERSION = "qwen_venice_app_v2_four_pass_crop_correction_v1"
CORRECTION_PROMPT_VERSION = "venice_app_v2_four_pass_crop_correction_prompt_v1"
...
PERSON_SCHEMA = obj(
    {
        ...
        "gaze_target_object_ref": nullable({"type": "string", "maxLength": 160}),
        "mouth_covered": nullable(enum(MOUTH_COVERED_VALUES)),
        "mouth_covering": nullable(enum(MOUTH_COVERING_CAUSES)),
        "mouth_covering_other_text": nullable({"type": "string", "maxLength": 160}),
...
GLOBAL_ADVERTISEMENT_SCHEMA = obj(
    {
        "advertisement_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*$"},
        "extent": enum(["full_page", "partial_page"]),
        "bbox_1000": BBOX_1000_SCHEMA,
        ...
        "duplicate_faces_present": nullable(enum(["yes", "no"])),
        "unique_face_count": nullable({"type": "integer", "minimum": 1, "maximum": 99}),
        "people": {"type": "array", "items": GLOBAL_PERSON_SCHEMA, "maxItems": 12},
        "identity_groups": {"type": "array", "items": IDENTITY_GROUP_SCHEMA, "maxItems": 12},
        "groups": {"type": "array", "items": GROUP_SCHEMA, "maxItems": 12},
        "confidence": CONFIDENCE_SCHEMA,
        "review_flags": REVIEW_FLAGS_SCHEMA,
...
GLOBAL_RESPONSE_SCHEMA = obj(
    {
        "schema_version": {"type": "string", "enum": [GLOBAL_SCHEMA_VERSION]},
        "playbook_version": {"type": "string", "enum": [PLAYBOOK_VERSION]},
        "page": obj(
            {
                "qualifying_ad_count": {"type": "string", "pattern": "^(0|[1-9][0-9]?)$"},
        ...
        "advertisements_truncated": {"type": "boolean"},
        "advertisements": {"type": "array", "items": GLOBAL_ADVERTISEMENT_SCHEMA, "maxItems": 20},
        "urgent_comments": {
            "type": "array",
            "items": obj(
                {
...
GLOBAL_VALIDATOR = Draft202012Validator(GLOBAL_RESPONSE_SCHEMA)

CROP_FEATURE_SCHEMA = obj(
    {
        "schema_version": {"type": "string", "enum": [CROP_SCHEMA_VERSION]},
        "playbook_version": {"type": "string", "enum": [PLAYBOOK_VERSION]},
        "advertisement_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*$"},
        "person_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*_person_[1-9][0-9]*$"},
        "crop_face_fit": enum(["tight_face_crop", "loose_includes_extra_body_or_background", "too_tight_or_cut_off", "wrong_region_or_no_face", "not_assessable"]),
        "suggest_rebox": {"type": "boolean"},
        "depiction_type": nullable(enum(DEPICTION_TYPES)),
        "perceived_age": nullable(enum(AGE_VALUES)),
        "perceived_gender_presentation": nullable(enum(GENDER_VALUES)),
        "face_expression_legibility": nullable(enum(EXPRESSION_LEGIBILITY_VALUES)),
...
CROP_VALIDATOR = Draft202012Validator(CROP_FEATURE_SCHEMA)

REPAIR_RESPONSE_SCHEMA = obj(
    {
        "schema_version": {"type": "string", "enum": [REPAIR_SCHEMA_VERSION]},
        "playbook_version": {"type": "string", "enum": [PLAYBOOK_VERSION]},
        "advertisement_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*$"},
        "person_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*_person_[1-9][0-9]*$"},
        "box_decision": enum(["replace_with_tighter_face_box", "keep_original_box", "not_an_individual_face"]),
        "face_box_within_crop_1000": nullable(BBOX_1000_SCHEMA),
        "confidence": CONFIDENCE_SCHEMA,
        "review_flags": REVIEW_FLAGS_SCHEMA,
    }
)

REPAIR_VALIDATOR = Draft202012Validator(REPAIR_RESPONSE_SCHEMA)

CORRECTION_RESPONSE_SCHEMA = obj(
    {
        "schema_version": {"type": "string", "enum": [CORRECTION_SCHEMA_VERSION]},
        "playbook_version": {"type": "string", "enum": [PLAYBOOK_VERSION]},
        "advertisement_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*$"},
        "person_id": {"type": "string", "pattern": "^ad_[1-9][0-9]*_person_[1-9][0-9]*$"},
        "correction_decision": enum(["replace_with_tighter_face_box", "keep_current_box", "wrong_region_or_no_face"]),
        "face_box_within_crop_1000": nullable(BBOX_1000_SCHEMA),
        "depiction_type": nullable(enum(DEPICTION_TYPES)),
        "perceived_age": nullable(enum(AGE_VALUES)),
        "perceived_gender_presentation": nullable(enum(GENDER_VALUES)),
...
CORRECTION_VALIDATOR = Draft202012Validator(CORRECTION_RESPONSE_SCHEMA)


def prompt_for_global_image(image_id: str, filename: str, page_type: str, year: int | None) -> str:
    schema_json = json.dumps(GLOBAL_RESPONSE_SCHEMA, ensure_ascii=True, separators=(",", ":"))
...
You are doing STEP 1 of a four-pass annotation of one full historical Economist magazine page.
Use only this page image and neutral image metadata. No human annotations, gold labels, or previous model outputs are available.
...
STEP 1 TASK
Find qualifying advertisements and locate eligible face depictions on the full page. Code page, advertisement, face-box, depiction-type, duplicate, gaze, and group fields that require full-page context.
Do not code per-face age, gender presentation, expression legibility, face orientation, mouth covering, smile presence, or smile intensity in this step; those will be coded from padded face crops after box repair.
...
- Include eligible faces even when expression is not readable.
...
- Count visible depictions, not unique real people.
...
FACE BOX AND GAZE CODING
- For 1 through 9 eligible faces in an ad: face_depiction_count_band is "1".."9"; draw every eligible individual face; groups=[]; has_outstanding_individuals=null.
- For 10 or more eligible faces: use "10_20" or "20_plus"; code only prominent outstanding individuals if present, and draw group boxes for remaining crowd/group faces.
- Face boxes must cover visible face and head features needed for later crop coding: visible hair, ears, forehead, cheeks, chin, beard or moustache, and face-worn items such as glasses. Exclude neck, shoulders, captions, labels, and empty background.
- depiction_type for an ad uses: {", ".join(DEPICTION_TYPES)}. If an ad has multiple depiction types, set ad.depiction_type="multiple_types_present" and set each person's depiction_type when known; otherwise person.depiction_type=null.
- gaze_target values: {", ".join(GAZE_VALUES)}. Code visible gaze direction from page context. Use gaze_target_object_ref only for advertised_product or other_object.

DUPLICATES AND GROUPS
- duplicate_faces_present is "yes" only when two or more individual boxes repeat the exact same represented face/identity with the same expression.
- Every duplicated face still gets a person record. The clearest main record stores duplicate_person_ids. Duplicate records set duplicate_of_person_id to the main person.
- For 10+ face ads, group boxes should include remaining faces in visually distinct groups, panels, scenes, or portrait sets.
- Code group fields from the full page using these allowed values: group_type={", ".join(GROUP_TYPE_VALUES)}; age_composition={", ".join(GROUP_AGE_VALUES)}; gender_presentation_composition={", ".join(GROUP_GENDER_VALUES)}; expression_legibility_distribution={", ".join(GROUP_LEGIBILITY_VALUES)}; dominant_gaze={", ".join(GROUP_GAZE_VALUES)}; smile_prevalence={", ".join(GROUP_SMILE_PREVALENCE_VALUES)}; dominant_smile_intensity={", ".join(GROUP_SMILE_INTENSITY_VALUES)}.
...
- Return exactly one object conforming to this JSON Schema.
...
def prompt_for_face_crop(
    image_id: str,
    filename: str,
    advertisement_id: str,
    person_id: str,
    ad_depiction_type: str,
    global_person_depiction_type: str | None,
) -> str:
    schema_json = json.dumps(CROP_FEATURE_SCHEMA, ensure_ascii=True, separators=(",", ":"))
    person_depiction_text = "null" if global_person_depiction_type is None else global_person_depiction_type
    return f"""
Return ONLY one valid compact JSON object. Do not use markdown fences, prose, comments, or hidden reasoning.
The first character must be {{ and the last character must be }}.

You are doing STEP 3 of a four-pass annotation. The image is a padded crop around exactly one face depiction found in the full-page pass after optional box repair.
Use only this crop and the neutral identifiers below. Do not infer brand, product category, race, ethnicity, actual identity, emotion, or smile authenticity.
...
- person_id: {person_id}
- global ad depiction_type: {ad_depiction_type}
- global person depiction_type, if any: {person_depiction_text}

STEP 3 TASK
Code only the individual facial feature fields visible in this crop: depiction_type if required/clear, perceived age, perceived gender presentation, expression legibility, face orientation, mouth covering, smile presence, and smile intensity.
Do not code gaze here; gaze was already coded from full-page context.
Also judge whether the crop is usable as a tight face/head crop. Set crop_face_fit="loose_includes_extra_body_or_background" when the crop still shows a full figure, torso, large body area, or mostly surrounding background. Set suggest_rebox=true whenever a tighter face/head box should be requested.

FIELD RULES
- If the ad depiction_type is not "multiple_types_present", set depiction_type=null unless the crop clearly contradicts the global ad depiction type and needs review.
...
- face_expression_legibility values: {", ".join(EXPRESSION_LEGIBILITY_VALUES)}. This measures how well the visible face supports expression coding.
- face_orientation values: {", ".join(FACE_ORIENTATION_VALUES)}.
- mouth_covered values: {", ".join(MOUTH_COVERED_VALUES)}.
- mouth_covering is null when mouth_covered="no". When mouth_covered is "yes" or "partly", choose one of: {", ".join(MOUTH_COVERING_CAUSES)}.
- mouth_covering_other_text is required only when mouth_covering="other"; otherwise null.
...
- If face_expression_legibility is "0_not_legible", set smile_present=null and smile_intensity=null. Still code face_orientation and mouth_covered if possible.
...
def prompt_for_bbox_repair(
    image_id: str,
    filename: str,
    advertisement_id: str,
    person_id: str,
    original_face_bbox_1000: list[int],
) -> str:
    schema_json = json.dumps(REPAIR_RESPONSE_SCHEMA, ensure_ascii=True, separators=(",", ":"))
    return f"""
Return ONLY one valid compact JSON object. Do not use markdown fences, prose, comments, or hidden reasoning.
The first character must be {{ and the last character must be }}.

You are doing STEP 2 of a four-pass annotation. The image is a generous crop around one candidate individual face/head box from a full-page pass.
...
- advertisement_id: {advertisement_id}
- person_id: {person_id}
- original full-page face_bbox_1000: {original_face_bbox_1000}

TASK
Decide whether the candidate box likely includes too much of the full figure/person rather than just the face/head. If so, return a tighter face/head box within THIS CROP using a 0..1000 coordinate grid relative to the crop image.

BOX RULES
- A valid individual box covers visible face and head features needed for coding: hair, ears, forehead, cheeks, chin, beard/moustache, glasses or face-worn items.
- Exclude neck, shoulders, torso, arms, legs, full figure, captions, labels, and empty background.
- If the current box is already a good face/head box, use box_decision="keep_original_box" and face_box_within_crop_1000=null.
- If the crop is not an individual eligible face region, use box_decision="not_an_individual_face" and face_box_within_crop_1000=null.
- Otherwise use box_decision="replace_with_tighter_face_box" and provide face_box_within_crop_1000.
...
def prompt_for_crop_correction(
    image_id: str,
    filename: str,
    advertisement_id: str,
    person_id: str,
    ...
    schema_json = json.dumps(CORRECTION_RESPONSE_SCHEMA, ensure_ascii=True, separators=(",", ":"))
    return f"""
Return ONLY one valid compact JSON object. Do not use markdown fences, prose, comments, or hidden reasoning.
The first character must be {{ and the last character must be }}.

You are doing STEP 4 of a four-pass annotation. A prior face-feature crop was flagged as loose, cut off, or wrong. The image shown here is that same crop.
...
TASK
Find the tightest visible face/head box for this same person within THIS CROP using a 0..1000 coordinate grid relative to the crop image, then code facial features from that corrected region.

BOX RULES
- A valid individual box covers visible face and head features needed for coding: hair, ears, forehead, cheeks, chin, beard/moustache, glasses or face-worn items.
- Exclude neck, shoulders, torso, arms, legs, full figure, captions, labels, and empty background.
- Use correction_decision="replace_with_tighter_face_box" when a tighter face/head box is visible and provide face_box_within_crop_1000.
- Use correction_decision="keep_current_box" only if the crop is already a tight face/head crop; then face_box_within_crop_1000=null.
- Use correction_decision="wrong_region_or_no_face" only if this crop does not contain an eligible face for this person; then face_box_within_crop_1000=null and set uncertain feature fields to not_assessable/null as required.

FEATURE RULES
- If the ad depiction_type is not "multiple_types_present", set depiction_type=null unless the crop clearly contradicts the global ad depiction type and needs review.
- perceived_age values: {", ".join(AGE_VALUES)}.
- perceived_gender_presentation values: {", ".join(GENDER_VALUES)}. Code visible presentation, not identity.
- face_expression_legibility values: {", ".join(EXPRESSION_LEGIBILITY_VALUES)}.
- face_orientation values: {", ".join(FACE_ORIENTATION_VALUES)}.
- mouth_covered values: {", ".join(MOUTH_COVERED_VALUES)}.
- mouth_covering is null when mouth_covered="no". When mouth_covered is "yes" or "partly", choose one of: {", ".join(MOUTH_COVERING_CAUSES)}.
- mouth_covering_other_text is required only when mouth_covering="other"; otherwise null.
- smile_present values: {", ".join(SMILE_PRESENT_VALUES)}. Code visible facial configuration only.
- smile_intensity is null unless smile_present="yes"; then use one of: {", ".join(SMILE_INTENSITY_VALUES)}.
- If face_expression_legibility is "0_not_legible", set smile_present=null and smile_intensity=null.
...
def validate_global_response(annotation: dict[str, Any]) -> list[str]:
    errors = [format_schema_error(error) for error in sorted(GLOBAL_VALIDATOR.iter_errors(annotation), key=str)]
    return errors


def validate_crop_response(annotation: dict[str, Any]) -> list[str]:
    errors = [format_schema_error(error) for error in sorted(CROP_VALIDATOR.iter_errors(annotation), key=str)]
    if not isinstance(annotation, dict):
        return errors
    if annotation.get("mouth_covered") == "no" and annotation.get("mouth_covering") is not None:
        errors.append("mouth_covered=no requires mouth_covering=null")
    if annotation.get("mouth_covered") in {"yes", "partly"} and annotation.get("mouth_covering") is None:
        errors.append("mouth covering yes/partly requires a cause")
    if annotation.get("mouth_covering") == "other" and not annotation.get("mouth_covering_other_text"):
        errors.append("mouth_covering=other requires mouth_covering_other_text")
    if annotation.get("mouth_covering") != "other" and annotation.get("mouth_covering_other_text") is not None:
        errors.append("mouth_covering_other_text must be null unless mouth_covering=other")
    if annotation.get("smile_present") == "yes" and annotation.get("smile_intensity") is None:
        errors.append("smile_present=yes requires smile_intensity")
    if annotation.get("smile_present") in {"no", "not_assessable"} and annotation.get("smile_intensity") is not None:
        errors.append("smile_present no/not_assessable requires smile_intensity=null")
    if annotation.get("face_expression_legibility") == "0_not_legible":
        for field in ["smile_present", "smile_intensity"]:
            if annotation.get(field) is not None:
                errors.append(f"0_not_legible requires {field}=null")
    return errors


def validate_repair_response(annotation: dict[str, Any]) -> list[str]:
    errors = [format_schema_error(error) for error in sorted(REPAIR_VALIDATOR.iter_errors(annotation), key=str)]
    if isinstance(annotation, dict):
        decision = annotation.get("box_decision")
        box = annotation.get("face_box_within_crop_1000")
        if decision == "replace_with_tighter_face_box" and box is None:
            errors.append("replace_with_tighter_face_box requires face_box_within_crop_1000")
        if decision != "replace_with_tighter_face_box" and box is not None:
            errors.append("face_box_within_crop_1000 must be null unless replacing")
    return errors


def validate_correction_response(annotation: dict[str, Any]) -> list[str]:
    errors = [format_schema_error(error) for error in sorted(CORRECTION_VALIDATOR.iter_errors(annotation), key=str)]
    if isinstance(annotation, dict):
        decision = annotation.get("correction_decision")
        box = annotation.get("face_box_within_crop_1000")
        if decision == "replace_with_tighter_face_box" and box is None:
            errors.append("replace_with_tighter_face_box requires face_box_within_crop_1000")
        if decision != "replace_with_tighter_face_box" and box is not None:
            errors.append("face_box_within_crop_1000 must be null unless replacing")
    errors.extend(validate_crop_response_like(annotation))
    return errors


def validate_crop_response_like(annotation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if not isinstance(annotation, dict):
        return errors
    if annotation.get("mouth_covered") == "no" and annotation.get("mouth_covering") is not None:
...
TASK
Apply annotation_playbook_v1.16 exactly. Use only the supplied image and neutral image metadata. No previous human or model annotations are available. Do not annotate brand, product category, ad topic, appeals, tone, emotion, race, ethnicity, actual identity, or authenticity of smiles.
...
- If qualifying ads are present: no_qualifying_ad_reason=null, and advertisements contains those ads in reading order.
- For a full-page single ad, extent="full_page" and bbox_1000=[0,0,1000,1000]. Otherwise bbox_1000 covers the complete advertising unit, including image and copy.
- Coordinates are integer [x1,y1,x2,y2] on a 0..1000 grid relative to the full page, including boxes for faces seen inside an advertisement crop.
...
- For each individual face, draw the smallest box covering all visible face and head features needed for coding: visible hair, ears, forehead, cheeks, chin, beard or moustache, and face-worn items such as glasses. Exclude neck, shoulders, captions, labels, and empty background.
...
- face_orientation values: {", ".join(FACE_ORIENTATION_VALUES)}.
- gaze_target values: {", ".join(GAZE_VALUES)}. Code visible gaze direction. If gaze_target="another_person" and the target has an individual box, set gaze_target_person_id to that id and gaze_target_person_unboxed=false. If the target is visible but has no individual box, set gaze_target_person_id=null and gaze_target_person_unboxed=true. Otherwise set both fields to null. Use gaze_target_object_ref only for a short visible-object reference when gaze_target is advertised_product or other_object; otherwise null.
- mouth_covered values: {", ".join(MOUTH_COVERED_VALUES)}.
- mouth_covering is null when mouth_covered="no". When mouth_covered is "yes" or "partly", choose one of: {", ".join(MOUTH_COVERING_CAUSES)}. mouth_covering_other_text is required only when mouth_covering="other"; otherwise null.
- smile_present values: {", ".join(SMILE_PRESENT_VALUES)}. Code visible facial configuration only.
- smile_intensity is null unless smile_present="yes"; then use one of: {", ".join(SMILE_INTENSITY_VALUES)}.
    ...
    for ad_index, ad in enumerate(ads):
        ...
        for person_index, person in enumerate(people):
            ...
            if person.get("mouth_covered") == "no" and person.get("mouth_covering") is not None:
                errors.append(f"{person_prefix}: mouth_covered=no requires mouth_covering=null")
            if person.get("mouth_covered") in {"yes", "partly"} and person.get("mouth_covering") is None:
                errors.append(f"{person_prefix}: mouth covering yes/partly requires a cause")
            if person.get("mouth_covering") == "other" and not person.get("mouth_covering_other_text"):
                errors.append(f"{person_prefix}: mouth_covering=other requires mouth_covering_other_text")
            if person.get("mouth_covering") != "other" and person.get("mouth_covering_other_text") is not None:
                errors.append(f"{person_prefix}: mouth_covering_other_text must be null unless mouth_covering=other")
            ...
            if person.get("gaze_target") == "another_person":
                ...
                unboxed = person.get("gaze_target_person_unboxed") is True
                if not target and not unboxed:
                    errors.append(f"{person_prefix}: another_person requires a target id or gaze_target_person_unboxed=true")
                elif target and unboxed:
                    errors.append(f"{person_prefix}: another_person cannot set both target id and unboxed=true")
                elif target and target not in person_ids:
                    errors.append(f"{person_prefix}: gaze_target_person_id does not refer to an existing person")
            elif person.get("gaze_target_person_id") is not None or person.get("gaze_target_person_unboxed") is not None:
                errors.append(f"{person_prefix}: non-person gaze requires person target fields=null")
            if person.get("gaze_target") not in {"advertised_product", "other_object"} and person.get("gaze_target_object_ref") is not None:
                errors.append(f"{person_prefix}: non-object gaze requires gaze_target_object_ref=null")
