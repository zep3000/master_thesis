"""Frozen schemas and prompts for the standalone final pipeline."""
...
PROMPT_VERSION = "final_standalone_v1"
...
STRUCTURE_SCHEMA = {
    ...
    "advertisements": [{
        ...
        "brand_or_advertiser": "visible or safely inferable printed historical name or null",
    ...
    if not include_business:
        for ad in schema["advertisements"]:
            for field in ["ad_category", "brand_or_advertiser"]:
                ad.pop(field, None)
    business = (
        "Classify the primary advertised offering or organization using the frozen WARC/Nielsen list, not incidental imagery. Business & industrial includes manufacturing, materials, construction, commercial equipment, and B2B services. Financial services includes insurance. Transport & tourism covers travel and transport services; passenger automotive is Automotive. Telecoms & utilities is separate from Technology & electronics. Media & publishing applies when a publication or media product is offered. For recruitment or corporate-image advertising with no direct offering, use the advertiser's main sector. Preserve the visible historical brand/advertiser name and use null rather than guessing. Category and brand inference must not change segmentation, face count, routing, or boxes."
...
Choose substantive age/gender/orientation/smile labels whenever evidence permits. Smile intensity is null unless smile_present=yes. Mouth covering is null unless mouth_covered is yes/partly. Return gaze and smile independently even when legibility is zero. The pipeline preserves raw normalized values in a sidecar and applies annotation-playbook nulls only to the canonical view.
...
Use the same 0-3 expression-legibility anchors as individual faces. Halftone/grain alone is not zero if visible configuration remains interpretable. all means all/nearly all; mostly means one level or category is a strict majority; mixed means no strict majority. `ambiguous_or_androgynous_present` requires a positively visible presentation and is not uncertainty. {gaze_task} Return gaze and smile independently even for all-zero legibility; the raw sidecar preserves them and canonical assembly applies required nulls. Intensity is null when smile prevalence is none/not_assessable.
Schema: {compact(group_schema(gaze))}"""


def duplicate_prompt(task_key: str, person_ids: list[str]) -> str:
    schema = {"task_key": task_key, "candidate_clusters": [{"person_ids": ["two or more supplied IDs"], "candidate_strength": "high or medium"}]}
    return f"""This is an isolated duplicate-candidate task over frozen face boxes and IDs. It cannot alter the main annotations.

Propose a link only when boxes reproduce the same underlying printed face depiction or artwork: expression, pose/angle, feature geometry, hair/occlusion pattern, and local image pattern must agree. The same represented person in a different photograph, frame, pose, angle, expression, or moment is not a duplicate. Adjacent people visible at a crop edge are context, not that crop's target. When in doubt, return no candidate.

Do not assess or output boxes, groups, counts, facial attributes, category, or brand. Every supplied face remains independently annotated. Supplied IDs: {json.dumps(person_ids)}

Return JSON matching {json.dumps(schema)}. If none qualify, return an empty candidate_clusters array."""
